...
class DocshopControllerOrders extends JControllerAdmin
{
    ...
    /**
     * Generate and stream a PDF invoice for a single order.
     * URL: index.php?option=com_docshop&task=orders.invoice&id=X&format=raw
     */
    public function invoice()
    {
        ...
        if (!$id) {
            $app->redirect(JRoute::_('index.php?option=com_docshop&view=orders', false), 'Invalid order ID.', 'error');
            return;
        }
        ...
        if (!$order) {
            $app->redirect(JRoute::_('index.php?option=com_docshop&view=orders', false), 'Order not found.', 'error');
            return;
        }

        // Bootstrap TCPDF from RSForm (already installed, no extra dependency)
        $tcpdfPath = JPATH_ADMINISTRATOR . '/components/com_rsform/helpers/tcpdf/tcpdf.php';
        if (!file_exists($tcpdfPath)) {
            $app->enqueueMessage('TCPDF not found at expected path.', 'error');
        ...
        // ---- Colours & fonts ----
        $blue   = array(30,  58, 138);   // #1e3a8a
        $light  = array(241, 245, 251);  // light blue-grey row bg
        $muted  = array(107, 114, 128);  // #6b7280
        $black  = array(17,  24,  39);   // #111827
        $white  = array(255, 255, 255);
        $green  = array(16, 185, 129);   // completed
        $amber  = array(245, 158, 11);   // pending
        $red    = array(239, 68,  68);   // failed
        $grey   = array(107, 114, 128);  // refunded
        ...
        $sc = isset($statusColor[$order->status]) ? $statusColor[$order->status] : $grey;
        ...
        if (!empty($order->paypal_transaction_id)) {
            $infoRows[] = array('Transaction', substr($order->paypal_transaction_id, 0, 28) . (strlen($order->paypal_transaction_id) > 28 ? '...' : ''));
        }
        ...
        $pdf->SetY(max($billToEndY, $pdf->GetY()) + 6);
        ...
        $pdf->SetTextColor($black[0], $black[1], $black[2]);
        $pdf->SetX($totX);
        $pdf->Cell($labelW, 7, 'TOTAL',   0, 0, 'R');
        $pdf->SetTextColor($blue[0], $blue[1], $blue[2]);
        $pdf->Cell($valueW, 7, ($order->currency ?: 'USD') . ' ' . number_format((float)$order->amount, 2), 0, 1, 'R');
